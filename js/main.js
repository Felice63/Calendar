...
// Make the Button 31 variable to control whether a month having 31 days instead of 30 sill display. This will be called in the conditional embedded in th callback
...
function showDate() {
  ...
  let formattedDate = `${dateInput.toLocaleDateString('en-US', { month: 'long' })} ${dateInput.getDate()+1} ${dateInput.getFullYear()}`;
  ...
  console.log(currentMonthNum, dateInput.getDate()+1)
...
// Set up the CSS grid to switch over to the column respresented dependent on the First day of the month; ie Sunday = 1, Monday = 2, etc. Need to set the start day of the month and add the grid-column class to the index of the month's start day, plus 1
  ...
  dayNumber.forEach((el) => {
    ...
    if(el.textContent == dateInput.getDate()+1) {
      console.log(el.textContent == dateInput.getDate()+1, `${el.textContent}, ${dateInput.getDate()+1}, ${el}, ${dayNumber}`);
...
// The function below parses the number of days in the specified month. It will be called dependent on the month selected from the stored dateInput value, inside the "showDate" callback above. The parameters `year`, and `month`, will be substituted with `monthNum` and `MonthYear` as arguments
